...
const NoteButton = ({
  ...
  const playAudio = useCallback(seconds => {
    const synth = new Tone.Synth().toDestination();
    synth.triggerAttackRelease(note, seconds);
  }, [note]);
  ...
  const onMouseUp = useCallback(() => {
    try {
      ...
      const dateDiff = initialDate.current.getTime() - actualDate.getTime();
      const diffInSeconds = Math.abs(dateDiff / 1000);
      playAudio(diffInSeconds);
    } catch (error) {
      playAudio(0.01);
    }
  }, []);
